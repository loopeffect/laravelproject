        ...
        <div class="container-xl">
            <div class="row g-2 items-center">
                <div class="col">
                    ...
                    <div class="mt-3 page-pretitle mb-2">
                        @if ($openai->type == 'code')
                ...
                <div class="col-sm-6 col-lg-5 lg:pr-14">
                    ...
                    <form class="row" id="openai_generator_form" onsubmit="return sendOpenaiGeneratorForm();">
                        ...
                        @if ($openai->type != 'code')
                            <div class="mb-[20px]">
                                <label class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="brand" name="brand">
                                    <span class="form-check-label">{{ __('Include Your Brand') }}</span>
                                </label>
                            </div>
                            ...
                            <div id="brandField" style="display:none">
                                <div class="mb-3">
                                    <div class="flex justify-between">
                                        ...
                                        <div class="outer-div p-1 mb-2 cursor-pointer"
                                            onclick="window.location.href='{{ LaravelLocalization::localizeUrl(route('dashboard.user.brand.edit')) }}'">
                                    ...
                                    <select class="form-select" id="company" name="company">
                                        ...
                                        @foreach (auth()->user()->getCompanies() as $company)
                                            <option data-tone_of_voice="{{ $company->tone_of_voice }}"
                                ...
                                <div class="mb-3">
                                    <div class="flex justify-between">
                                        <label class="form-label">{{ __('Select Product/Service') }}</label>
                                    </div>
                        ...
                        @foreach (json_decode($openai->questions) as $question)
                            <div class="mb-3 col-xs-12 @if ($question->type == 'rss_feed') relative @endif">
                                @php
                                ...
                                @elseif($question->type == 'textarea')
                                    ...
                                    <textarea class="form-control" id="{{ $question->name }}" name="{{ $question->name }}" rows="12"
                                        placeholder="{{ __($placeholder) }}" maxlength="{{ $setting->openai_max_input_length }}" required="required"></textarea>
                                ...
                                @elseif($question->type == 'rss_feed')
                                    ...
                                    <input type="{{ $question->type }}" class="form-control" id="{{ $question->name }}"
                                        ...
                                        placeholder="{{ __($placeholder) }}" required="required">
                                    <div
                                        class="fetch-rss absolute right-0 bottom-[1px] leading-[45px] pr-6 pl-4 border-l border-r-0 border-b-0 border-t-0 border-solid border-[var(--tblr-border-color)] cursor-pointer hover:scale-95">
                        ...
                        @if ($openai->type == 'youtube')
                            <div class="mb-3 col-xs-12">
                                ...
                                <select class="form-select" id="youtube_action" name="youtube_action"
                                    required="required">
                        ...
                        @if ($openai->type == 'text' || $openai->type == 'rss')
                            ...
                            <div class="mb-3 col-xs-12 col-md-6">
                                ...
                                <input type="number" class="form-control" id="number_of_results"
                                    name="number_of_results" value="1" placeholder="{{ __('Number of results') }}"
                                    required>
                            </div>
...
@section('script')
    ...
    {{-- <script>
        ...
        tinymce.init({
            selector: '#myTextarea',
            plugins: 'wordcount',
            toolbar: 'undo redo | formatselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | removeformat | wordcount',
            setup: function(editor) {
                // Add mousemove event listener
                editor.on('mousemove', function(e) {
                    // Check if the target is a paragraph element
                    if (e.target.nodeName === 'P') {
                        // Highlight the paragraph using jQuery
    ...
    <script>
        var brandCheckbox = document.getElementById('brand');
        var brandField = document.getElementById('brandField');
        brandCheckbox.addEventListener('change', function() {
            brandField.style.display = brandCheckbox.checked ? '' : 'none';
        });
